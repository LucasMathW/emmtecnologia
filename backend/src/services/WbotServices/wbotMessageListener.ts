...
const getContactMessage = async (msg: WAMessageSafe, wbot: Session) => {
  ...
  const lid =
    key.sender_lid && key?.sender_lid.includes("@lid")
      ? key.sender_lid
      : key.participant_lid && key?.participant_lid.includes("@lid")
      ? key.participant_lid
      : key.remoteJid && key?.remoteJid.includes("@lid")
      ? key.remoteJid
      : null;
  const senderPn =
    key.remoteJidAlt && key.remoteJidAlt.length > 0
      ? key.remoteJidAlt
...
export const verifyMediaMessage = async (
  ...
  logger.info(
    `[MEDIA-START] Iniciando processamento de mídia | msgId: ${
      msg.key.id
    } | type: ${getTypeMessage(msg)} | ticketId: ${ticket.id}`
  );
  ...
  try {
    ...
    while (downloadAttempts < maxAttempts && !media?.data) {
      try {
        ...
        logger.info(
          `[MEDIA-DOWNLOAD] Iniciando tentativa ${downloadAttempts} para msgId: ${msg.key.id}`
        );
        ...
        if (!media?.data) {
          logger.warn(
            `[MEDIA-DOWNLOAD] Tentativa ${downloadAttempts} falhou - media.data é null/undefined | msgId: ${msg.key.id}`
          );
          media = null;
        } else {
          logger.info(
            `[MEDIA-DOWNLOAD] Sucesso na tentativa ${downloadAttempts} | msgId: ${
              msg.key.id
            } | tamanho: ${media.data?.length || 0} bytes`
          );
        }
      } catch (downloadError) {
        logger.warn(
          `[MEDIA-DOWNLOAD] Erro na tentativa ${downloadAttempts}: ${downloadError.message} | msgId: ${msg.key.id}`
        );
        media = null;
      }
    }
    ...
    // ✅ LOG: Verificar se é WebP animado
    const isSticker =
    ...
    try {
      ...
      const filePath = join(folder, media.filename);
      ...
      logger.error(
        `[MEDIA-SAVE-ERROR] Erro ao salvar mídia: ${err.message} | msgId: ${msg.key.id}`
      );
    }
    ...
    logger.info(
      `[MEDIA-DONE] Mídia processada com sucesso | msgId: ${msg.key.id} | messageId: ${newMessage.id} | filename: ${media.filename}`
    );
    ...
    logger.warn(
      `[MEDIA-ERROR] Erro ao baixar media: ${error.message} | msgId: ${msg.key.id}`
    );
  }
};
...
const verifyQueue = async (
  ...
  const botText = async () => {
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (
        queue?.randomizeImmediate ||
        (settings?.settingsUserRandom === "enabled" && randomUserId)
      ) {
        await UpdateTicketService({
        ...
        console.log(
          `[IMMEDIATE RANDOMIZATION] Ticket ${ticket.id} atribuído imediatamente para usuário ${randomUserId}`
        );
      }
...
export const flowbuilderIntegration = async (
  ...
  logger.info(
    `[RDS-FLOW-DEBUG] flowbuilderIntegration iniciado para ticket ${ticket.id}`
  );
  logger.info(
    `[RDS-FLOW-DEBUG] Parâmetros: queueIntegration.type=${queueIntegration?.type}, flowWebhook=${ticket.flowWebhook}, lastFlowId=${ticket.lastFlowId}`
  );
  ...
  if (ticket.flowWebhook && ticket.lastFlowId && msg) {
    ...
    if (queueIntegration?.type === "flowbuilder" && !ticket.userId) {
      logger.info(
        `[RDS-FLOW-DEBUG] Forçando início do fluxo para ticket ${ticket.id}, mesmo com flowWebhook=${ticket.flowWebhook}`
      );
    } else {
      logger.info(
        `[RDS-FLOW-DEBUG] Ticket ${ticket.id} já em fluxo ativo (lastFlowId: ${ticket.lastFlowId}), ignorando nova verificação de campanha`
      );
      return false;
    }
  }
  ...
  if (msg && msg.key.fromMe) {
    logger.info(
      `[RDS-FLOW-DEBUG] Mensagem é fromMe, ignorando fluxo para ticket ${ticket.id}`
    );
    return false;
  }
  ...
  if (messageId && (await cacheLayer.get(cacheKey))) {
    console.log(
      `[CAMPANHAS] Mensagem ${messageId} já processada para campanhas no ticket ${ticket.id}`
    );
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[CAMPANHAS] ❌ Erro ao limpar estado do ticket:",
        cleanupError
      );
    }
  }
  ...
  const isNewContact =
    contact.createdAt &&
    Math.abs(new Date().getTime() - new Date(contact.createdAt).getTime()) <
  ...
  if (firstContactAlreadyRan) {
    console.log(
      `[FIRST CONTACT] ⏭️ Já executado anteriormente para ticket ${ticket.id}, ignorando.`
    );
  }
  ...
  logger.info(
    `[FLOW CHECK] ========== VERIFICANDO SE DEVE CONTINUAR FLUXO ==========`
  );
  ...
  logger.info(
    `[FLOW CHECK] Condição (flowWebhook && hashFlowId): ${!!(
      ticket.flowWebhook && ticket.hashFlowId
    )}`
  );
  ...
  if (ticket.flowWebhook && ticket.hashFlowId) {
    ...
    try {
      ...
      if (webhook && webhook.config["details"]) {
        ...
        if (msg && msg.key.fromMe) {
          logger.info(
            `[FLOW WEBHOOK] ⚠️ Mensagem é do bot (fromMe=true) - IGNORANDO para evitar loop`
          );
          logger.info(
            `[FLOW WEBHOOK] Aguardando resposta do USUÁRIO para ticket ${ticket.id}`
          );
          return false;
        }

        logger.info(`[FLOW WEBHOOK] ========== WEBHOOK ENCONTRADO ==========`);
        ...
        if (msg && msg.key.fromMe) {
          logger.info(
            `[FLOW STOPPED] ⚠️ Mensagem é do bot (fromMe=true) - IGNORANDO para evitar loop`
          );
          logger.info(
            `[FLOW STOPPED] Aguardando resposta do USUÁRIO para ticket ${ticket.id}`
          );
          return false;
        }
        ...
        logger.info(
          `[FLOW STOPPED] Ticket ${ticket.id}, FlowId: ${ticket.flowStopped}`
        );
        ...
        if (flow) {
          ...
          logger.info(
            `[FLOW STOPPED] Chamando ActionsWebhookService com pressKey: "${body}"`
          );
      ...
      logger.error("[FLOW WEBHOOK] ❌ Erro ao processar fluxo webhook:", error);
    }
  } else {
    logger.warn(
      `[FLOW CHECK] ❌ CONDIÇÃO NÃO ATENDIDA - Fluxo não será continuado`
    );
    logger.warn(
      `[FLOW CHECK] Motivo: flowWebhook=${ticket.flowWebhook}, hashFlowId=${
    ...
    if (ticket.flowStopped && ticket.lastFlowId) {
      logger.error(
        `[FLOW CHECK] ⚠️ PROBLEMA DETECTADO: Ticket tem flowStopped=${ticket.flowStopped} e lastFlowId=${ticket.lastFlowId}`
      );
      logger.error(
        `[FLOW CHECK] mas flowWebhook=${ticket.flowWebhook} e hashFlowId=${
          ticket.hashFlowId || "null"
        }`
      );
      logger.error(
        `[FLOW CHECK] Isso indica que o ticket não foi salvo corretamente após enviar o menu!`
      );
    }
  }
...
export const handleMessageIntegration = async (
  ...
  const notifyIntegrationErrorAndReset = async (
    ticketReset: Ticket,
    companyResetId: number,
    bodyText?: string
  ) => {
    const fallbackText =
      bodyText ||
    ...
    try {
      ...
      logger.error(
        `[INTEGRATION FALLBACK] Erro ao enviar mensagem de falha: ${sendErr?.message}`
      );
    }
    ...
    try {
      ...
      logger.info(
        `[INTEGRATION FALLBACK] Ticket ${ticketReset.id} finalizado e integração limpa.`
      );
    } catch (updateErr) {
      logger.error(
        `[INTEGRATION FALLBACK] Erro ao finalizar/limpar ticket ${ticketReset.id}: ${updateErr?.message}`
      );
    }
  };
  ...
  try {
    ...
    try {
      ...
      let cfg: any = {};
      cfg = queueIntegration?.jsonContent
        ? JSON.parse(queueIntegration.jsonContent)
        : {};
      if (
        queueIntegration.type === "SGP" ||
        ((cfg?.sgpUrl || cfg?.tipoIntegracao) &&
          queueIntegration.type !== "typebot")
      ) {
      ...
      /* ignora parse e segue */
    }

    if (
      queueIntegration.type === "n8n" ||
      queueIntegration.type === "webhook"
    ) {
    ...
    logger.error(
      `[INTEGRATION ERROR] Erro ao processar integração ${queueIntegration?.type} no ticket ${ticket.id}:`,
      error
    );
  ...
  try {
    ...
    if (isGroup) {
      ...
      try {
        ...
        logger.error(
          `Erro ao obter metadados do grupo: ${JSON.stringify(error)}`
        );
      }
      ...
      if (!grupoMeta) {
        try {
          ...
          grupoMeta = await getGroupMetadataCache(
            whatsapp.id,
            msg.key.remoteJid
          );
        } catch (error) {
          logger.error(
            `Erro ao atualizar cache do grupo: ${JSON.stringify(error)}`
          );
        }
      }
      ...
      if (!groupContact) {
        logger.info(
          "Grupo não encontrado, buscando novamente no banco de dados..."
        );
        ...
        if (!groupContact) {
          logger.info(
            "Grupo não encontrado, descarta a mensagem para não abrir como contato..."
          );
          return;
        }
      }
    ...
    if (!isGroup && !msg.key.fromMe) {
      try {
        ...
        wbot.presenceSubscribe(jidParaSubscribe).catch(err => {
          logger.warn(`[PRESENCE] Erro ao subscribe: ${err.message}`);
        });
    ...
    if (
      ...
      if (isAIMode && dataWebhook?.settings) {
        logger.info(
          `[AI MODE] Processando mensagem em modo ${dataWebhook.type} - ticket ${ticket.id}`
        );
        ...
        try {
          ...
          if (dataWebhook.awaitingUserResponse) {
            logger.info(
              `[AI SERVICE] Primeira resposta do usuário para ${dataWebhook.type} - iniciando conversa - ticket ${ticket.id}`
            );
    ...
    const wasProcessedByTemporaryAI = await checkTemporaryAI(
      wbot,
      ticket,
      contact,
      msgContact,
    ...
    if (
      ...
      try {
        ...
        const resolvedFlowId =
          ticket.flowStopped || (ticket.dataWebhook as any)?.flowId;
    ...
    if (ticket.flowStopped && ticket.lastFlowId) {
      ...
      if (msg && msg.key.fromMe) {
        logger.info(
          `[FLOW STOPPED] ⚠️ Mensagem do bot (fromMe=true) - IGNORANDO para ticket ${ticket.id}`
        );
        return;
      }

      logger.info(
        `[FLOW STOPPED] ========== CONTINUANDO FLUXO (SEGUNDA VERIFICAÇÃO) ==========`
      );
      logger.info(
        `[FLOW STOPPED] Ticket ${
          ticket.id
        }, Mensagem do usuário: "${getBodyMessage(msg)}"`
      );
...
const wbotMessageListener = (wbot: WbotSession, companyId: number): void => {
    ...
    for (const contact of contacts) {
      ...
      try {
        ...
        if (contact.imgUrl === undefined || contact.imgUrl === null) {
          console.log(`[PIC-EVENT] Sem imgUrl para ${number} — ignorando`);
          continue;
        }
  ...
  wbot.ev.on("group-participants.update", async event => {
    ...
    if (action === "add") {
      logger.info(
        `Adicionando participantes ao grupo ${id}, atualizando cache`
      );
      ...
      logger.info(
        `Atualizando ${
          action === "promote" ? "admin" : "participante"
        } do grupo ${id}, atualizando cache`
      );
  ...
  wbot.ev.on("groups.update", (groupUpdate: GroupMetadata[]) => {
    groupUpdate.forEach(async (group: GroupMetadata) => {
      ...
      try {
        const picCacheKey = `pic:${companyId}:${group.id}`;
