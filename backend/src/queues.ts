    ...
    await Promise.all(
      companies.map(async company => {
        try {
          ...
          await Promise.all(
            company.whatsapps.map(async whatsapp => {
              ...
              try {
                ...
                for (const [index, message] of pendingMessages.entries()) {
                  try {
                    ...
                    const whatsappInstance = await Whatsapp.findByPk(
                      conexao.id
                    );
                    if (!whatsappInstance) {
                      logger.error(
                        `Whatsapp não encontrado para o ID: ${whatsapp.id}`
                      );
                      continue;
                    }
                      ...
                      try {
                        ...
                        if (message.path) {
                          if (fs.existsSync(message.path)) {
                            fileBuffer = await fs.promises.readFile(
                              message.path
                            );
                          } else {
                            logger.warn(
                              `Arquivo de mídia não encontrado: ${message.path}`
                            );
                          }
                        }
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    if (schedule.reminderDate && schedule.reminderStatus === "PENDENTE") {
      logger.info(
        `Agendamento ${schedule.id} tem lembrete configurado - não enviando mensagem no horário original`
      );
    ...
    if (schedule.openTicket === "enabled") {
      ...
      let bodySchedule = bodyMessage.replace(
        "{{dataAgendamento}}",
        dataAgendamento
      );
      ...
      const dataAgendamento = await agendamentoContato(schedule);
      let bodySchedule = schedule.body.replace(
        "{{dataAgendamento}}",
        dataAgendamento
      );
...
export const agendamentoContato = async (
  schedule: Schedule
): Promise<string> => {
  try {
...
async function handleVerifyCampaigns(job) {
  ...
  try {
    ...
    if (campaigns.length > 0) {
      ...
      const promises = campaigns.map(async campaign => {
        try {
          ...
          if (!result || result.length === 0) {
            logger.info(
              `Campanha ${campaign.id} não está mais disponível para processamento`
            );
            return null;
          }
          ...
          if (delay < 1000) {
            logger.info(
              `Campanha ${campaign.id} deve executar imediatamente (delay muito pequeno)`
            );
      ...
      const validPromises = (await Promise.all(promises)).filter(
        p => p !== null
      );
...
async function getCampaign(id) {
  ...
  if (campaign.tagListId && !campaign.contactListId) {
    logger.info(
      `[TAG-DEBUG] Buscando contatos por tagId: ${campaign.tagListId} para campanha: ${id}, companyId: ${campaign.companyId}`
    );
    ...
    logger.info(
      `[TAG-DEBUG] ContactTags encontrados para tagId ${campaign.tagListId}:`,
      contactTags.map(ct => ({
    ...
    logger.info(
      `[TAG-DEBUG] ContactIds encontrados para tagId ${campaign.tagListId}:`,
      contactIdList
    );
    ...
    logger.info(
      `[TAG-DEBUG] Contatos encontrados via Contact.findAll:`,
      contacts.map(c => ({
        id: c.id,
        name: c.name,
        number: c.number,
    ...
    for (const contact of contacts) {
      ...
      logger.info(
        `[TAG-DEBUG] Contato ${contact.id} (${contact.name}) tem as tags:`,
        tagIds
      );

      if (!tagIds.includes(Number(campaign.tagListId))) {
        logger.error(
          `[TAG-DEBUG] ERRO: Contato ${contact.id} (${contact.name}) não deveria estar na tag ${campaign.tagListId}!`
        );
      }
    }

    logger.info(
      `[TAG-DEBUG] Total de ${contacts.length} contatos encontrados para tag ${campaign.tagListId}`
    );
...
async function getContact(id, campaignId = null) {
  logger.info(
    `[RDS-CAMPAIGN-DEBUG] Buscando contato com ID: ${id}, Campaign ID: ${campaignId}`
  );
  ...
  if (isTagCampaign) {
    ...
    if (contact) {
      logger.info(
        `[RDS-CAMPAIGN-DEBUG] Contato encontrado em Contact (campanha por tag): ${
          contact.name
        } (Company: ${contact.companyId || "N/A"})`
      );
      return contact;
    }

    logger.error(
      `[RDS-CAMPAIGN-DEBUG] ERRO: Contato com ID ${id} não encontrado na tabela Contact para campanha por tag (Company filter: ${
        companyId || "none"
      })`
    );
    return null;
  ...
  if (contactListItem) {
    logger.info(
      `[RDS-CAMPAIGN-DEBUG] Contato encontrado em ContactListItem: ${contactListItem.name}`
    );
  ...
  if (contact) {
    logger.info(
      `[RDS-CAMPAIGN-DEBUG] Contato encontrado em Contact (fallback): ${
        contact.name
      } (Company: ${contact.companyId || "N/A"})`
    );
    return contact;
  ...
  logger.error(
    `[RDS-CAMPAIGN-DEBUG] ERRO: Contato com ID ${id} não encontrado em nenhuma tabela (Company filter: ${
      companyId || "none"
    })`
  );
  return null;
...
async function verifyAndFinalizeCampaign(campaign) {
  ...
  logger.info(
    `[VERIFY CAMPAIGN] Campanha ${campaign.id}: ${deliveredCount} mensagens entregues de ${contacts.length} contatos, ${realExecutionCount} execuções reais vs ${campaign.executionCount} registradas`
  );
  logger.info(
    `[VERIFY CAMPAIGN] Campanha ${campaign.id}: isRecurring=${campaign.isRecurring}, maxExecutions=${campaign.maxExecutions}, status=${campaign.status}`
  );

  if (realExecutionCount > campaign.executionCount) {
    logger.info(
      `[VERIFY CAMPAIGN] Corrigindo executionCount da campanha ${campaign.id} de ${campaign.executionCount} para ${realExecutionCount}`
    );
    ...
    if (
      ...
      realExecutionCount >= campaign.maxExecutions
    ) {
      logger.info(
        `[VERIFY CAMPAIGN] Campanha ${campaign.id} atingiu limite de ${campaign.maxExecutions} execuções após correção - finalizando`
      );
      await campaign.update({
        status: "FINALIZADA",
        completedAt: moment()
  ...
  if (currentExecutionCount > campaign.executionCount) {
    ...
    logger.info(
      `[RDS-VERIFY CAMPAIGN] Campanha ${campaign.id} executionCount atualizado para: ${currentExecutionCount}`
    );
  }
  ...
  if (campaign.isRecurring) {
    ...
    if (
      campaign.maxExecutions &&
      currentExecutionCount >= campaign.maxExecutions
    ) {
      logger.info(
      ...
      logger.info(
        `[RDS-VERIFY CAMPAIGN] Campanha ${
          campaign.id
        } é recorrente - agendando próxima execução (${currentExecutionCount}/${
          campaign.maxExecutions || "ilimitado"
        })`
      );
    ...
    if (deliveredCount >= contacts.length) {
      logger.info(
        `[RDS-VERIFY CAMPAIGN] Campanha ${campaign.id} não é recorrente - todas as ${deliveredCount} mensagens foram entregues - finalizando`
      );
      await campaign.update({
        status: "FINALIZADA",
        completedAt: moment()
...
async function handleProcessCampaign(job) {
  try {
    const {
      id,
      restartMode,
      messageInterval: customMessageInterval,
      longerIntervalAfter: customLongerIntervalAfter,
      greaterInterval: customGreaterInterval
    }: ProcessCampaignData = job.data;
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        if (restartMode) {
          ...
          const deliveredNumbersSet = new Set(
            deliveredNumbers.map(d => d.number)
          );

          // Filtrar apenas contatos pendentes
          const pendingContacts = contacts.filter(
            contact => !deliveredNumbersSet.has(contact.number)
          );
        ...
        const messageInterval = restartMode
          ? customMessageInterval || 20
          : settings.messageInterval;
        const longerIntervalAfter = restartMode
          ? customLongerIntervalAfter || 20
          : settings.longerIntervalAfter;
        const greaterInterval = restartMode
          ? customGreaterInterval || 60
          : settings.greaterInterval;

        if (contactData.length === 0) {
          console.log(
            `[PROCESS-CAMPAIGN] Nenhum contato pendente encontrado para campanha ${campaign.id}`
          );
          return;
        }
        ...
        console.log(
          `[CAMPAIGN] ${queuePromises.length} jobs adicionados à fila para campanha ${campaign.id}`
        );
        await Promise.all(queuePromises);
      } else {
        console.log(
          `[PROCESS-CAMPAIGN] ERRO: Lista de contatos não é um array para campanha ${campaign.id}`
        );
      }
    }
  } catch (err: any) {
...
function calculateDelay(
  ...
  console.log(
    `[CALCULATE-DELAY] Index: ${index}, DiffSeconds: ${diffSeconds}, FinalDelay: ${finalDelay}ms`
  );
...
async function handlePrepareContact(job) {
  try {
    ...
    if (!contact) {
      logger.error(
        `[CAMPAIGN] Contato ${contactId} não encontrado para campanha ${campaignId}`
      );
      return;
    }
    ...
    if (!contact.number) {
      logger.error(
        `[CAMPAIGN] Contato ${contactId} (${
          contact.name || "sem nome"
        }) não possui número de telefone`
      );
      return;
    }
    ...
    if (
      ...
      const nextJob = await campaignQueue.add("DispatchCampaign", {
        campaignId: campaign.id,
...
async function handleDispatchCampaign(job) {
  try {
    ...
    if (!wbot) {
      logger.error(
        `[CAMPAIGN] Wbot não encontrado para campanha ${campaignId}`
      );
      return;
    }
    ...
    if (!campaign.whatsapp) {
      logger.error(
        `[CAMPAIGN] WhatsApp não encontrado para campanha ${campaignId}`
      );
      return;
    }
    ...
    if (!wbot?.user?.id) {
      logger.error(
        `[CAMPAIGN] Usuário do wbot não encontrado para campanha ${campaignId}`
      );
      return;
    }
    ...
    if (!campaignShipping) {
      logger.error(
        `[CAMPAIGN] CampaignShipping ${campaignShippingId} não encontrado`
      );
      return;
    }
    ...
    if (campaignShipping.contact && campaignShipping.contact.isGroup) {
      ...
      const isGroupNumber =
        campaignShipping.number.includes("@") ||
        campaignShipping.number.length > 15;
    ...
    if (campaign.openTicket === "enabled") {
      ...
      if (whatsapp.status === "CONNECTED") {
        if (campaign.confirmation && campaignShipping.confirmation === null) {
          ...
          if (!campaign.mediaPath) {
            console.log(
              `[DISPATCH-CAMPAIGN] Enviando mensagem de texto para: ${chatId}`
            );
            console.log(
              `[DISPATCH-CAMPAIGN] Conteúdo da mensagem: ${campaignShipping.message}`
            );
            ...
            console.log(
              `[DISPATCH-CAMPAIGN] Mensagem enviada com sucesso: ${
                sentMessage ? "SIM" : "NÃO"
              }`
            );
      ...
      if (campaign.confirmation && campaignShipping.confirmation === null) {
        ...
        if (!campaign.mediaPath) {
          console.log(
            `[DISPATCH-CAMPAIGN] Enviando mensagem SEM ticket para: ${chatId}`
          );
          console.log(
            `[DISPATCH-CAMPAIGN] Conteúdo da mensagem: ${campaignShipping.message}`
          );
          ...
          console.log(
            `[DISPATCH-CAMPAIGN] Mensagem enviada com sucesso (sem ticket): ${
              sentMessage ? "SIM" : "NÃO"
            }`
          );
        }
    ...
    logger.info(
      `Campanha enviada para: Campanha=${campaignId};Contato=${
        campaignShipping.contact
          ? campaignShipping.contact.name
...
async function handleRandomUser() {
  ...
  const jobR = new CronJob("0 */1 * * * *", async () => {
    try {
          ...
          c.queues.map(async q => {
            ...
            const findUserById = async (userId, companyId) => {
              try {
                ...
                logger.error(
                  `[VerifyUsersRandom] VerifyUsersRandom: error ${JSON.stringify(
                    errorV
                  )}`
                );
                throw errorV;
            ...
            if (count > 0) {
              for (const ticket of tickets) {
                ...
                if (
                  ...
                  const hasWalletForQueue = ticket.contact.contactWallets.some(
                    wallet => wallet.queueId === queueId
                  );

                  if (hasWalletForQueue) {
                    logger.info(
                      `[RANDOM USER] Ticket ${ticket.id} possui carteira definida para fila ${queueId} - pulando randomização`
                    );
      ...
      Sentry.captureException(e);
      logger.error(
        `[VerifyUsersRandom] VerifyUsersRandom: error ${JSON.stringify(e)}`
      );
      throw e;
    }
  });
                ...
                if (
                  ...
                  if (nextTag.mediaFiles) {
                    try {
                      ...
                      console.log(
                        "Error sending media files in auto lane movement:",
                        error
                      );
                    }
                  }
          ...
          if (status === true) {
            ...
            if (dias <= -3) {
              ...
              if (openInvoices.length > 0) {
                ...
                logger.info(
                  `Fatura Atualizada ID: ${invoiceToUpdate.id} com valor ${valuePlan}`
                );
              } else {
...
async function handleLidRetry(job) {
  try {
    ...
    const {
      ...
      retryCount,
      maxRetries = 5
    } = data as LidRetryData;

    logger.info(
      `[RDS-LID-RETRY] Tentativa ${retryCount} de obter LID para contato ${contactId} (${number})`
    );
    ...
    if (!contact) {
      logger.error(
        `[RDS-LID-RETRY] Contato ${contactId} não encontrado. Cancelando retentativa.`
      );
      return;
    }
    ...
    if (!whatsapp || whatsapp.status !== "CONNECTED") {
      logger.error(
        `[RDS-LID-RETRY] WhatsApp ${whatsappId} não está conectado. Reagendando retentativa.`
      );
      ...
      if (retryCount < maxRetries) {
        ...
        logger.warn(
          `[RDS-LID-RETRY] Número máximo de tentativas (${maxRetries}) atingido para contato ${contactId}. Desistindo.`
        );
      }
      return;
    }
    ...
    try {
      ...
      if (!wbot) {
        throw new Error(
          `Instância WhatsApp ${whatsappId} não encontrada no wbot`
        );
      }
      ...
      if (ow?.[0]?.exists) {
        ...
        if (lid) {
          logger.info(
            `[RDS-LID-RETRY] LID ${lid} obtido com sucesso para contato ${contactId}`
          );
          ...
          logger.info(
            `[RDS-LID-RETRY] Mapeamento de LID criado/atualizado para contato ${contactId}`
          );
          return;
        }
      }
      ...
      logger.warn(
        `[RDS-LID-RETRY] Não foi possível obter LID para contato ${contactId} (${number})`
      );
      ...
      if (retryCount < maxRetries) {
        ...
        logger.info(
          `[RDS-LID-RETRY] Reagendada tentativa ${
            retryCount + 1
          } para contato ${contactId}`
        );
      } else {
        logger.warn(
          `[RDS-LID-RETRY] Número máximo de tentativas (${maxRetries}) atingido para contato ${contactId}. Desistindo.`
      ...
      logger.error(
        `[RDS-LID-RETRY] Erro ao processar retentativa para contato ${contactId}: ${error.message}`
      );
    ...
    Sentry.captureException(err);
    logger.error(
      `[RDS-LID-RETRY] Erro geral no processador de retentativas: ${err.message}`
    );
  }
}
...
async function handleSendMessageOficial(job) {
  try {
    ...
    if (!media && payload?.downloadUrl) {
      ...
      fsAny.mkdirSync(
        pathAny.join(publicFolder, `company${ticket.companyId}`),
        { recursive: true }
      );

      const response = await axiosAny.get(payload.downloadUrl, {
