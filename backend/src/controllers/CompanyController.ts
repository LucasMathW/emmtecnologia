...
const calculateDirectoryMetrics = async (
  companyId: number
): Promise<DirectoryMetrics> => {
  ...
  try {
    ...
    console.error(
      `Error calculating directory metrics for company ${companyId}:`,
      error
    );
    const result: DirectoryMetrics = {
      folderSize: 0,
...
export const indexPlan = async (
  ...
  if (requestUser.super === true) {
    try {
          ...
          return {
            ...
            metrics: {
              ...
              lastUpdate: metrics.lastUpdate
                ? moment(metrics.lastUpdate).format("DD/MM/YYYY HH:mm:ss")
                : null
            }
          };
        })
      ...
      return res
        .status(500)
        .json({ error: "Erro ao buscar dados das empresas" });
    }
  } else {
    return res
      .status(400)
