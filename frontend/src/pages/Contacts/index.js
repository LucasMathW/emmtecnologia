...
const Contacts = () => {
  ...
  const handleSelectAllContacts = () => {
    if (selectAllMode) {
      ...
      if (
        allCurrentSelected &&
        selectedContacts.size === currentPageIds.length
      ) {
  ...
  const handleBulkDeleteConfirm = async () => {
    try {
      if (deleteType === "all") {
        ...
        toast.success(
          `Todos os ${totalContactsCount} contatos foram excluídos`,
        );
      } else {
  ...
  useEffect(() => {
    ...
    const onContactEvent = (data) => {
      ...
      if (data.action === "bulk-delete") {
        ...
        setTotalContactsCount((prev) =>
          Math.max(0, prev - data.contactIds.length),
        );
      }
      ...
      if (data.action === "delete-all") {
        dispatch({
          ...
          payload: { excludeIds: data.excludeIds },
        });
  ...
  const getConfirmAction = () => {
    ...
    if (unBlockingContact)
      return () => handleUnBlockContact(unBlockingContact.id);
  ...
  const getConfirmTitle = () => {
    if (deletingContact)
      return `${i18n.t("contacts.confirmationModal.deleteTitle")} ${deletingContact.name}?`;
    if (blockingContact) return `Bloquear Contato ${blockingContact.name}?`;
    if (unBlockingContact)
      return `Desbloquear Contato ${unBlockingContact.name}?`;
    if (ImportContacts)
      return i18n.t("contacts.confirmationModal.importTitlte");
  ...
  const getConfirmMessage = () => {
    if (exportContact)
      return i18n.t("contacts.confirmationModal.exportContact");
    if (deletingContact)
      return i18n.t("contacts.confirmationModal.deleteMessage");
    if (blockingContact)
      return i18n.t("contacts.confirmationModal.blockContact");
    if (unBlockingContact)
      return i18n.t("contacts.confirmationModal.unblockContact");
    if (ImportContacts)
      return i18n.t("contacts.confirmationModal.importMessage");
        ...
        {isAnyContactSelected && (
          <Toolbar className={`${classes.toolbar} ${classes.toolbarHighlight}`}>
            ...
            >
              ...
              {selectAllMode && "(TODOS OS CONTATOS)"}
            </Typography>
            <div className={classes.bulkActions}>
              ...
              >
                <IconButton color="inherit" onClick={handleBulkDeleteClick}>
                  <DeleteOutlineIcon />
                </IconButton>
              </Tooltip>
            </div>
          ...
          <TableBody>
            <>
              {contacts.map((contact) => {
                const isSelected =
                  selectAllMode || selectedContacts.has(contact.id);

                return (
                  <TableRow key={contact.id} selected={isSelected}>
                    <TableCell className={classes.checkboxCell}>
                      <Checkbox
                        checked={isSelected}
                      ...
                      {enableLGPD && hideNum && user.profile === "user"
                        ? contact.isGroup
                          ...
                          : formatSerializedId(contact?.number) === null
                            ...
                            : formatSerializedId(contact?.number)?.slice(
                                0,
                                -6,
                              ) +
                              "**-**" +
                    ...
                    <TableCell align="center">
                      {contact.contactWallets &&
                      contact.contactWallets.length > 0
                        ? contact.contactWallets[0].wallet?.name ||
