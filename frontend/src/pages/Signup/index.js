...
const SignUp = () => {
  ...
  useEffect(() => {
    // Backend resolve empresa pelo domínio — sem companyId
    ...
    getPublicSetting("appLogoBackgroundLight")
      .then((bgLight) => {
        setBackgroundLight(
          bgLight ? getBackendUrl() + "/public/" + bgLight : "",
        );
      })
      .catch(() => setBackgroundLight(""));

    getPublicSetting("appLogoBackgroundDark")
      .then((bgDark) => {
  ...
  const validateDocument = async (document, setFieldValue) => {
    ...
    if (documentType === "cpf") {
      ...
      try {
        const response = await openApi.post("/auth/validate-cnpj", {
          cnpj: cleanDocument,
        });
        ...
        if (data.valid && data.data.nome) {
          if (data.data.tipo === "cnpj") {
            ...
            toast.success(
              "CNPJ válido! Nome da empresa preenchido automaticamente.",
            );
          } else {
            toast.success("CPF válido!");
          }
          ...
          setLastValidatedDocument(cleanDocument);
          toast.error(
            "Documento inválido ou não encontrado na Receita Federal",
          );
          return false;
        }
      } catch (error) {
  ...
  if (mode === "light") {
    if (hasValidLightBg) {
      ...
      finalBackground = theme.palette.background
        ? theme.palette.background.default
        : "#f5f5f5";
    }
  } else {
  ...
  return (
    <React.Fragment>
      <Helmet>
        <title>{appName || "AtendeChat - Cadastro"}</title>
