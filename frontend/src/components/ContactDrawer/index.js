...
const ContactDrawer = ({
  ...
  const [participantInitialContact, setParticipantInitialContact] =
    useState(undefined);
  ...
  const searchMessages = async (searchParam) => {
    ...
    try {
      const { data } = await api.get(
        `/contacts/${contact.id}/messages/search`,
        {
      ...
      <List className={classes.participantsList}>
        {participants.map((participant) => (
          <ListItem
            ...
            button
            onClick={() => handleOpenChatWithParticipant(participant)}
          >
            <ListItemAvatar>
              <Badge
                overlap="circular"
            ...
            <ListItemSecondaryAction>
              <IconButton
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleOpenChatWithParticipant(participant);
                }}
              >
                <ChatIcon />
              </IconButton>
  ...
  const handleOpenChatWithParticipant = async (participant) => {
    try {
      ...
      if (!contactId || contactId === 0) {
        const createResp = await api.post("/contacts", {
      ...
      >
        ...
        <div className={classes.contentWrapper}>
          ...
          <Box className={classes.switchContainer}>
            <Typography style={{ marginBottom: 0 }} variant="subtitle2">
              <Switch
                size="small"
              ...
              icon={
                <Box display="flex" alignItems="center">
                  ...
                  {mediaData.documents.length > 0 && (
                    <Chip
                      size="small"
                      label={mediaData.documents.length}
                      className={classes.tabChip}
                    />
                  )}
                </Box>
